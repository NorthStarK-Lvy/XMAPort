...
def init_console():
    ...
    os.system("")
    # 设置控制台窗口标题
    os.system("title XMAPort 260801.Beta")
    # 防止非 UTF-8 终端下中文输出崩溃
    try:
...
def pack_one_partition(part, fs_dir, pack_cfg, lpc_args, counters):
    ...
    # system_ext + is_skip_apex=true 时用源 payload 镜像替换
    if part == "system_ext" and pack_cfg.get("is_skip_apex", "false").lower() == "true":
        src_img = SRC_UNPACK / "system_ext.img"
        if src_img.exists():
            try:
                shutil.copy2(src_img, out_img)
                info("is_skip_apex=true: system_ext.img replaced by source payload image")
                log_write("system_ext.img replaced by source payload (is_skip_apex=true)")
            except Exception:
                err("is_skip_apex=true but overwrite failed")
                log_write("WARNING: system_ext.img overwrite failed")
        else:
            err("is_skip_apex=true but source system_ext.img not found")
            log_write("WARNING: source system_ext.img not found for skip_apex")
...
def one_click_port():
    ...
    # 传递打包环境变量给 pack_partitions.py（UTC 时间戳 / erofs 旧内核兼容 / 旧版 erofs 工具）
    ...
    os.environ["XMAPORT_EROFS_LEGACY_EXE"] = str(pack_cfg.get("if_legacy_erofs", "false"))
...
def main():
    ...
    while True:
        ...
        if ch == "1":
            ...
            os.system("cls" if os.name == "nt" else "clear")
            print("  {}============================================================{}".format(C, N))
            print("  {}  Done HyperOS Porting!{}".format(G, N))
            print("  {}============================================================{}".format(C, N))
            pause()
            return
        else:
